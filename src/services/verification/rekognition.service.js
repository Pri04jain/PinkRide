...
const mockEnsureCollection = async () => ({ created: true });

// Internal functions for real AWS calls
const _detectAndValidateFace = detectAndValidateFace;
const _indexFace = indexFace;
const _compareFace = compareFace;
const _deleteFace = deleteFace;
const _ensureCollection = ensureCollection;

module.exports = {
  ensureCollection: () => isMockMode() ? mockEnsureCollection() : _ensureCollection(),
  detectAndValidateFace: (buf) => isMockMode() ? mockDetect(buf) : _detectAndValidateFace(buf),
  indexFace: (buf, userId) => isMockMode() ? mockIndex(buf, userId) : _indexFace(buf, userId),
  compareFace: (buf, faceId) => isMockMode() ? mockCompare(buf, faceId) : _compareFace(buf, faceId),
  deleteFace: (faceId) => isMockMode() ? mockDelete(faceId) : _deleteFace(faceId),
  MATCH_THRESHOLD,
};
