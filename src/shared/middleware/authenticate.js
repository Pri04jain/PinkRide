...
const authenticate = async (req, res, next) => {
  try {
    ...
    if (decoded.jti) {
      ...
      // Wrapped in try/catch so a missing table doesn't crash every request
      try {
        ...
        // Only rethrow if it's our own AppError (token revoked)
        // Ignore DB errors (e.g. table not yet created) — treat as not revoked
        if (revokeErr instanceof AppError) throw revokeErr;
        console.warn('[Auth] revoked_tokens check skipped:', revokeErr.message);
      }
    }
